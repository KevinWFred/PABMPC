...
# o select samples within 5 years of follow-up time, I used the following code to avoid LIID overlap between repeated samples:
#   y5_1 <- data[data$SAMPLE_TYPE==1&data$follow_time<=5,]  #358 samples
#   y5_2 <- data[data$SAMPLE_TYPE==2&data$follow_time<=5,]  #270 samples
#   y5_2 <- y5_2[!y5_2$LIID %in% y5_1$LIID,]                                           #166 samples
#   y5 <- rbind(y5_1, y5_2)
#   
#   In total, we have 69 case/control sets within 5 years of follow-up time in ATBC and 193 (110 plus 83 [NOT 88 BECAUSE SOME FIRST SAMPLES WERE AT T1 BUT NOT T0]) case/control sets within 5 years of follow-up time in PLCO.
library(pROC)
library(boot)
...
#alldat=read.csv("../data/Dataset for LASSO/Dataset for LASSO-12Jun24.csv")
...
y5_2 <- y5_2[!y5_2$LIID %in% y5_1$LIID,]                                           #166 samples
y5 <- rbind(y5_1, y5_2)
table(y5$COHORT,y5$follow_time<=5)
#      TRUE
# ATBC  138
# PLCO  386
table(y5$COHORT,y5$CASE)
#        0   1
# ATBC  69  69
# PLCO 193 193
#only work on 5year data
alldat=y5
library(glmnet)
###need to specify your input file for dataset
#alldat0=read.csv("../data/Dataset for LASSO/Dataset for LASSO-7Jun23.csv")
#change race 
...
allmetabolite=alldat[,24:ncol(alldat)]
...
#[1] 472
#2 cohorts
ATBCdat=alldat[alldat$COHORT=="ATBC",]
PLCOdat=alldat[alldat$COHORT=="PLCO",]
...
get3AUC=function(testdat,predictY=predictY_min)
{
  ...
  #sometimes there are few diabetes 1, can't get CI
  boot_auc=tryCatch({
    boot(data =testdat, statistic = AUCBoot1, R = 1000)
  ...
  boot_auc=tryCatch({
    boot(data =testdat, statistic = AUCBoot3, R = 1000)
  },error=function(e) NULL)
  if (!is.null(boot_auc))
  {
    tmp=boot.ci(boot_auc,type="perc")
...
#seedopt=2: for trainPLCO5years, CI is not stable, pick seed 2000
computeAUC=function(traindat=ATBCdat,testdat=PLCOdat,selmarkers=allmarkers,prefix="ATBC",nyear=10,seedopt=1)
{
  ...
  #elastic net
  traindat=traindat[traindat$follow_time<=nyear,]
  trainX=traindat[,selmarkers]
  trainY=traindat$CASE
  testdat=testdat[testdat$follow_time<=nyear,]
  testX=testdat[,selmarkers]
  testY=testdat$CASE
  pairid=traindat$CELL_SET
  foldid=generate_foldid(X=traindat,Y=trainY,pairid,nfolds=3)
  set.seed(1000)
  ...
  resall=list(auc=auc,coeffs_min=coeffs_min,coeffs_refit=coeffs_refit,
              predictY_min=predictY_min,trainres=trainres,testres=testres,
              predictrisk=predictrisk,predictcombined=predictcombined)
  #to save weights
  weightfile=paste0("../result/",prefix,"_elsticnetweights_rmunknown_year",nyear,".csv")
  tmp=coeffs_min
...
trainATBC_nyear5=computeAUC(nyear=5)
#"AUC: riskfactors vs combined p:0.297235895548126"
#"AUC: marker vs combined p:0.0775160245627346"
#"AUC: marker vs intercept (AUC 0.5) p:0.000744072616161238"
#"AUC: riskfactros vs intercept (AUC 0.5) p:0.000192457480073748"
#"AUC: combined vs intercept (AUC 0.5) p:5.56934315342879e-05"
trainATBC_nyear5$auc
#                  risk           maker         combine
# elsticnet 0.6(0.55,0.66) 0.59(0.53,0.65) 0.61(0.57,0.68)
# refit     0.6(0.54,0.66) 0.62(0.56,0.67) 0.62(0.57,0.68)
trainPLCO_nyear5=computeAUC(traindat=PLCOdat,testdat=ATBCdat,prefix="PLCO",nyear=5,seedopt=2)
#"AUC: riskfactors vs combined p:0.000390011755963011"
#"AUC: marker vs combined p:0.118341282203607"
#"AUC: marker vs intercept (AUC 0.5) p:1.48587096161167e-08"
#"AUC: riskfactros vs intercept (AUC 0.5) p:0.0652751389794735"
#"AUC: combined vs intercept (AUC 0.5) p:1.25662048152095e-10"
trainPLCO_nyear5$auc
#                  risk           maker         combine
# elsticnet 0.57(0.46,0.68) 0.73(0.65,0.81) 0.76(0.69,0.84)
# refit         0.57(NA,NA) 0.73(0.65,0.81)     0.76(NA,NA)

save(trainATBC_nyear5,trainPLCO_nyear5,PLCOdat,ATBCdat,file="../result/elasticnet_2cohorts_5_new.RData")
...
#cex: font size
plotroc3=function(predict1=trainATBC_nyear5$predictrisk,predict2=trainATBC_nyear5$predictY_min[,1],predict3=trainATBC_nyear5$predictcombined,y=PLCOdat$CASE[which(PLCOdat$follow_time<=10)],main="",
                  prefix="trainATBC_year5",aucres=NULL,cex=1.3)
{
  ...
  }else
  {
    ...
    legend=gsub("(NA,NA)","",legend,fixed = T)
  }
...
plotroc3(predict1=trainATBC_nyear5$predictrisk,predict2=trainATBC_nyear5$predictY_min[,1],predict3=trainATBC_nyear5$predictcombined,y=PLCOdat$CASE[which(PLCOdat$follow_time<=5)],main="",
                  prefix="trainATBC_year5_new",aucres=NULL,cex=1.3)
plotroc3(predict1=trainPLCO_nyear5$predictrisk,predict2=trainPLCO_nyear5$predictY_min[,1],predict3=trainPLCO_nyear5$predictcombined,y=ATBCdat$CASE[which(ATBCdat$follow_time<=5)],main="",
         prefix="trainPLCO_year5_new")

#include CI in AUC
plotroc3(predict1=trainATBC_nyear5$predictrisk,predict2=trainATBC_nyear5$predictY_min[,1],predict3=trainATBC_nyear5$predictcombined,y=PLCOdat$CASE[which(PLCOdat$follow_time<=5)],
         aucres = trainATBC_nyear5,prefix="trainATBC_year5_CI_new",cex=1.4)
plotroc3(predict1=trainPLCO_nyear5$predictrisk,predict2=trainPLCO_nyear5$predictY_min[,1],predict3=trainPLCO_nyear5$predictcombined,y=ATBCdat$CASE[which(ATBCdat$follow_time<=5)],main="",
         prefix="trainPLCO_year5_CI_new",aucres = trainPLCO_nyear5,cex=1.4)
# #not include CI
